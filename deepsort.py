...
class deepsort_rbc():
    def __init__(self, wt_path=None):
        ...
        if wt_path is not None:
            self.encoder = torch.load(wt_path,'cpu')
        else:
    ...
    def run_deep_sort(self, frame, out_scores, out_boxes ,labels = None):
        ...
        if labels != None:
            dets = [(dets[i],labels[i]) for i in indices]
        else:
            dets = [(dets[i], None) for i in indices]
